...
## Дополнение после block-02 (2026-09-23)

- Чужой миграции `.claude/` в окне уже не было, зато в `git diff HEAD --stat` попали файлы самого плана (`docs/plans/<component>/*`) и память агентов (`.claude/agent-memory/*`). Их строки входят в общий итог, но кодом и документацией продукта не являются — выносить отдельной оговоркой после итоговой суммы.
- Untracked в этом окне оказались отчёты аудитов в `docs/reviews/*.md` — их тоже приводить через `wc -l` с пометкой untracked.
- Число, переданное вызывающей стороной, может отставать от реальности (ему называли 47 файлов, `rtk git diff HEAD --stat` на момент написания дал 58/3385/192). Всегда перемерять самому и указывать, что замер сделан при написании отчёта.
