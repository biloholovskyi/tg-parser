...
## Дополнение после block-08-09 (2026-09-24)

- Файлы фаз могут все стоять в `Статус: todo`, пока индекс плана пишет `done` — статус брать из индекса и фактов вызывающей стороны, расхождение оговаривать под таблицей Phases.
- Число тестов от вызывающей стороны бывает приблизительным («538+»); перезапускать `rtk npm.cmd test`, `test:e2e`, `typecheck` самому и писать наблюдённое. `lint` не перезапускать — он с автоисправлением и меняет файлы.
- Отчёт аудита может помечать пункт неисправленным, а файл к моменту отчёта уже исправлен — проверять grep'ом и писать фактическое состояние.
