...
Статус: done (кроме дымового прогона — за пользователем)
...
- [x] Проверка типов: `rtk npm run typecheck`
- [x] Аудит рефакторинга и безопасности через агент `security-auditor`: подтверждено отсутствие учётных данных на диске
- [x] Аудит ресурсов через агент `resource-leak-auditor`: владение клиентом не потерялось при перемещении кода
- [ ] Дымовой прогон: полный цикл авторизации и выборка постов на поднятом сервисе
- [x] Проверка дрейфа через агент `docs-drift-auditor`: правила, `CLAUDE.md` и ADR описывают одно и то же
- [x] Артефакты ручного тестирования обновлены, если форма ответа изменилась
